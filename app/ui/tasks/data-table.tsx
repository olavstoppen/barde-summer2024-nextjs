  ...
  return (
    <>
      ...
      <div className='flex items-center justify-between'>
        ...
        <DropdownMenu>
          ...
          {types.map((item: string) => {
             ...
             onCheckedChange={() => table.getColumn('type')?.getFilterValue() === item?
               table.getColumn('type')?.setFilterValue("") : table.getColumn('type')?.setFilterValue(item)
             }
           >
             {item}
           </DropdownMenuCheckboxItem>
          ...
          <DropdownMenuContent align='start'>
            {machines.map((item: string) => {
              return(
                 ...
                 onCheckedChange={() => table.getColumn('machine')?.getFilterValue() === item?
                   table.getColumn('machine')?.setFilterValue("") : table.getColumn('machine')?.setFilterValue(item)
                 }
               >
                 {item}
               </DropdownMenuCheckboxItem>
      ...
      <div className='rounded-lg border'>
        <Table className='rounded-lg bg-pri-cont'>
              ...
              table.getRowModel().rows.map((row: { id: React.Key | null | undefined; getIsSelected: () => any; getVisibleCells: () => any[] }) => (
                ...
                >
                  ...
                  {row.getVisibleCells().map(cell => (
                    <TableCell key={cell.id} className="bg-gray-100">
                      {flexRender(
